...
const getOauth = (key: string, secret: string) => {
  return new OAuth({
    consumer: { key, secret },
    signature_method: 'HMAC-SHA1',
    hash_function(base_string: string, k: string) {
...
export const requestTokenLogic = async (
  db: Firestore,
  consumerKey: string,
...
export const callbackLogic = async (
  ...
  oauth_verifier: string | null,
    ...
    if (tokenDoc.exists) {
      ...
      request_token_secret = data?.secret || null;
    }
